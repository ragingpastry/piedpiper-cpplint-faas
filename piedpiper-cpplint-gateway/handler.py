from .util import request_new_task_id, call_cpplint_function, update_task_id_status
...
def handle(request):
    ...
    return_value = call_cpplint_function(run_id=run_id,
                                        faas_url="http://172.17.0.1:8080/async-function/piedpiper-cpplint-function",
                                        project=project,
    ...
    if return_value.status_code == 202:
        update_task_id_status(gman_url=gman_url, task_id=internal_task_id,
                              status='delegated', message='Delegated execution to cpplint executor')
